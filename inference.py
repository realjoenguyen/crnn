...
def test(net, data, abc, visualize, batch_size, num_workers=0,
    ...
    data_loader = DataLoader(data, batch_size=batch_size, num_workers=num_workers, shuffle=False,
    ...
    if visualize:
        ...
        for name, pred, gt, dist, _ in log[:10]:
            print(name, repr(pred), repr(gt), dist)
...
def main():
    ...
    data = TextDataset(data_path=config.data_path, mode=config.test_mode, transform=transform)
    input_size = [int(x) for x in config.input_size.split('x')]
